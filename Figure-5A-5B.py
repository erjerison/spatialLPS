...
def plot_power_spectra_cdfs_and_summary(output_name,mode='Log',data_src='Counts'):
	...
	for sample in sample_list:
		...
		print(sample)
		if sample in ['d-08192022_LPS25ugml_10hrs_tail4', 'd-08092022_LPS30ugml_10hrs_tail3', 'd-08092022_LPS30ugml_4hrs_tail2']:
		...
		for g in gene_inds_all:
			...
			frac_var_ax.plot(gene_counter + .1*numpy.random.random()-.05, frac_var,
					 marker='o',alpha=.85,color=color)
			...
			if g in gene_inds_examples:
				...
				#ax_cdf.semilogx(smoothed_eigs, cum_frac, color=color, linewidth=2, alpha=.85)
				ax_cdf.plot(numpy.arange(1,len(smoothed_frac)+.5),cum_frac_perm,color='grey',linewidth=1,alpha=.5)
				#ax_cdf.semilogx(smoothed_eigs, cum_frac_perm, color='grey', linewidth=1, alpha=.5)
				ax_cdf.set_xlabel('Mode Number',fontsize=14)
				#ax_cdf.set_xlabel('$1/\\lambda$ ($\\mu m^{-1}$)',fontsize=16)
				...
				if axes_counter < .5:
					...
					#ax_cdf.text(0.0006,1,'A.',fontsize=24)
	...
	print('frac_vars shape:', frac_vars.shape)
...
if __name__=='__main__':
	...
	plot_power_spectra_cdfs_and_summary(output_name='alt2-Fig5-variance-explained',data_src='Intensities',mode='Log')
	plot_power_spectra_cdfs_and_summary(output_name='alt2-FigS12-variance-explained',data_src='Counts',mode='Log')
