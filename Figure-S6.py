...
def plot_expression_unfolded(output_name,data_src='Counts'):
	...
	treatment_to_index = {'LPS25ugml_10hrs':0, 'LPS27ugml_10hrs':1,'LPS30ugml_4hrs':2,'LPS30ugml_10hrs':3}
	...
	for sample in sample_list:
		...
		spatial_location_filename = paths_filenames.table_path + '/'+ sample + '/' + sample + '-uv_centroids.npy'
		...
		sorter_list.append([treatment_to_index[treatment_dict[sample]], numpy.mean(data_mat)])
	...
	sample_order = numpy.argsort([row for row in sorter_list], axis=0 )[:,0]
	...
	for siter in range(n_samples):
		...
		spatial_location_filename = paths_filenames.table_path + '/'+ sample + '/' + sample + '-uv_centroids.npy' ##Note that this file contains the coordinates using the Coons patch transformation
...
if __name__=='__main__':
	...
	plot_expression_unfolded('alt-FigureS7-rectangle-expression-with-means',data_src='Counts')
	plot_expression_unfolded('alt-Figure3-rectangle-expression-with-means',data_src='Intensities')
