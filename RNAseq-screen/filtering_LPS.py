...
input_file = data_path + 'LPS_counts_cont_scores.h5ad.gz' ###Loom file generated by load_and_estimate_quality_lps.Rmd
output_file = data_path + 'LPS_counts_filtered.h5ad.gz'
...
pt.savefig('figures/lps_decontX_score_hist.pdf')
pt.figure()
pt.hist(adata.obs['doublet_score'].values,log=True,bins=numpy.arange(0,1,.02))
pt.savefig('figures/lps_doublet_score_hist.pdf')
...
sc.tl.umap(adata)
sc.tl.leiden(adata)
sc.pl.umap(adata, color=['leiden','decontX_scores','doublet_score','il1b','and2','krt4','prss1'],save='lps_umap1_with_doublet_predictions.pdf')
...
sc.tl.umap(adata)
sc.tl.leiden(adata)
sc.pl.umap(adata, color=['leiden','decontX_scores','doublet_score','il1b','and2','krt4','prss1'],save='lps_umap2_with_contaminant_filtered.pdf')
...
adata = adata[adata.obs.doublet_score < .3,:]
adata = adata[adata.obs.decontX_scores < .6,:]
...
sc.tl.umap(adata)
sc.tl.leiden(adata)
sc.pl.umap(adata, color=['leiden','decontX_scores','doublet_score','il1b','and2','krt4','prss1'],save='lps_umap1_decontX_counts.pdf')
