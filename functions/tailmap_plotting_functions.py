...
def plot_cell_intensities_diverging_cm(label_image_filename, component_arr, axis_obj, sample, cm='coolwarm',alpha_bg=.2, scalecolor='k'):
	...
	axis_obj.imshow(intensity_image1,alpha=alpha_image,cmap=cm,vmin=numpy.percentile(component_arr,1),vmax=numpy.percentile(component_arr,99),zorder=1)
	axis_obj.imshow(background_image,cmap='YlOrBr',alpha=alpha_bg,zorder=2)
	axis_obj.plot([150, 390], [300, 300], linestyle='-', color=scalecolor, linewidth=3)  # Scale bar: 20 microns assuming 0.12 um per pixel
	axis_obj.set_xticks([])
	axis_obj.set_yticks([])



def plot_cell_intensities_diverging_cm_nan(label_image_filename, component_arr, axis_obj, sample, cm='coolwarm',alpha_bg=.2, scalecolor='k'):
	...
	intensity_image1[label_image==0] = numpy.nan

	masked_array = numpy.ma.array(intensity_image1, mask=numpy.isnan(intensity_image1))
	cmap = pt.cm.get_cmap(cm)
	#cmap.set_bad(color='w',alpha=0.)
	cmap.set_bad(color=[0.1,0.1,0.1],alpha=0.)

	axis_obj.imshow(intensity_image1,alpha=alpha_image,cmap=cmap,vmin=numpy.percentile(component_arr,1),vmax=numpy.percentile(component_arr,99),zorder=1)
	#axis_obj.imshow(background_image,cmap='YlOrBr',alpha=alpha_bg,zorder=2)
	axis_obj.plot([150, 390], [300, 300], linestyle='-', color=scalecolor, linewidth=3)  # Scale bar: 20 microns assuming 0.12 um per pixel
	axis_obj.set_xticks([])
	axis_obj.set_yticks([])